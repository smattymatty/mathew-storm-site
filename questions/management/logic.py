...
def _create_or_update_question(q_data: Dict[str, Any], tutorial_title_instance: TutorialTitle, file_path_str: str) -> Dict[str, bool]:
    """Helper to create or update a single question."""
    question_id_slug = q_data.get('question-id')
    if not question_id_slug:
        logger.warning(f"Skipping question due to missing 'question-id' in {file_path_str}. Data: {q_data}")
    ...
    defaults = {
        ...
        'correct_answer': str(q_data.get('correct_answer', '')).lower(), # Ensure lowercase
        'difficulty': str(q_data.get('difficulty', Question.Difficulty.EASY)).lower(), # Ensure lowercase
    }
    ...
    try:
        ...
        if isinstance(tags_list, list):
            ...
            logger.warning(f"Tags data for question '{question_id_slug}' in '{file_path_str}' is not a list. Skipping tags update.")
        
        return {"created": created, "updated": not created, "skipped_malformed": False}

    except IntegrityError as e:
        logger.error(f"IntegrityError for question '{question_id_slug}' in '{file_path_str}': {e}. Data: {defaults}")
        return {"created": False, "updated": False, "skipped_malformed": True} # Count as malformed due to integrity
    except Exception as e:
        logger.error(f"Unexpected error creating/updating question '{question_id_slug}' in '{file_path_str}': {e}")
        return {"created": False, "updated": False, "skipped_malformed": True} # Or a new 'failed' category
...
def process_single_json_file(file_path: Path, overall_stats: Dict) -> None:
    ...
    for q_data in questions_data_list:
        ...
        if not title_id_slug:
            logger.warning(f"Skipping question entry due to missing 'title-id' in {file_path}. Data: {q_data}")
        ...
        try:
            ...
            with transaction.atomic():
                tutorial_title_instance, tt_created = TutorialTitle.objects.get_or_create(
                    title_id_slug=title_id_slug,
                    defaults={'name': q_data.get('title_name', title_id_slug)}
                )
                if tt_created:
                    overall_stats['titles_created'] += 1
                    logger.debug(f"Created TutorialTitle: {title_id_slug}")

                result = _create_or_update_question(q_data, tutorial_title_instance, str(file_path))
            ...
            logger.error(f"Critical error processing an entry in {file_path} for title '{title_id_slug}': {e}")
            file_questions_skipped += 1 # Count as skipped due to critical error
    ...
    logger.info(f"Finished processing {file_path}: {file_questions_created} created, {file_questions_updated} updated, {file_questions_skipped} skipped.")


def load_questions_from_json_files(json_file_paths: List[Path]) -> Dict[str, Any]:
    """
